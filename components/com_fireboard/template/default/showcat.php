...
if($letPass || $is_Mod){
	...
	$database->setQuery("SELECT a. * , MAX( b.time )  AS lastpost FROM  #__fb_messages  AS a LEFT  JOIN #__fb_messages  AS b ON b.thread = a.thread WHERE a.parent =  '0' AND a.catid =  $catid AND a.hold =  '0' GROUP  BY id ORDER  BY ordering DESC , lastpost DESC  LIMIT $offset,$threads_per_page");
			...
			if(count($messages[0]) > 0){
				...
				for($i = ($page - 2) <= 0 ? 1 : ($page - 2); $i <= $page + 2 && $i <= ceil($total / $threads_per_page); $i++){
					if($page == $i){
						...
						echo '<li class="jr-pagenav-nb"><a  class="jr-pagenav-nb" href="' . sefRelToAbs(JB_LIVEURLREL . '&amp;func=showcat&amp;catid=' . $catid . '&amp;page=' . $i) . '">' . $i . '</a></li>';
					}
				}
				if($page + 2 < ceil($total / $threads_per_page)){
					echo "<li class=\"jr-pagenav-nb\">...&nbsp;</li>";
					echo
						'<li class="jr-pagenav-nb"><a  class="jr-pagenav-nb" href="' . sefRelToAbs(JB_LIVEURLREL . '&amp;func=showcat&amp;catid=' . $catid . '&amp;page=' . ceil($total / $threads_per_page)) . '">' . ceil($total / $threads_per_page) . '</a></li>';
				}
...
<?php
	...
	if(count($messages) > 0){
		if($view == "flat") if(file_exists(JB_ABSTMPLTPATH . '/flat.php')){
					...
					<table class="fb_blocktable" id="fb_bottomarea" border="0" cellspacing="0" cellpadding="0" width="100%">
						...
						<tr>
							<th class="th-left" align="left">
								...
								if($my->id != 0){
									...
									echo $fbIcons->markThisForumRead ? '<img src="' . JB_URLICONSPATH . '' . $fbIcons->markThisForumRead . '" border="0" alt="' . _GEN_MARK_THIS_FORUM_READ . '" title="' . _GEN_MARK_THIS_FORUM_READ . '"/>' : _GEN_MARK_THIS_FORUM_READ;
									echo '</a>';
								}
								?>
								<!-- Mod List -->
						...
						<tr class="<?php echo $boardclass;?>sectiontableentry1">
							<td class="td-1 fbs" align="left">
								...
								<?php
								if($moderatedForum == 1){
									echo $fbIcons->forummoderated ? '<img src="' . JB_URLICONSPATH . '' . $fbIcons->forummoderated . '" border="0" alt="' . _GEN_MODERATED . '" /> - ' . _GEN_MODERATED . '' : '  <img src="' . JB_URLEMOTIONSPATH . 'review.gif" border="0"  alt="' . _GEN_MODERATED . '" /> - ' . _GEN_MODERATED . '';
								} else{
									echo "";
								}
								?>
							...
							<td class="td-2 fbs" align="left">
								...
								if($topicLocked){
									echo
									$fbIcons->topiclocked ? '<img src="' . JB_URLICONSPATH . '' . $fbIcons->topiclocked . '" border="0" alt="' . _GEN_LOCKED_TOPIC . '" title="' . _GEN_LOCKED_TOPIC . '" /> - ' . _GEN_LOCKED_TOPIC . '' : '<img src="' . JB_URLEMOTIONSPATH . 'lock.gif" alt="' . _GEN_LOCKED_TOPIC . '" title="' . _GEN_LOCKED_TOPIC . '" /> - ' . _GEN_LOCKED_TOPIC . '';
								}
								?>
								<br/>
								<?php
								if($topicSticky){
									echo $fbIcons->topicsticky ? '<img src="' . JB_URLICONSPATH . '' . $fbIcons->topicsticky . '" border="0" alt="' . _GEN_ISSTICKY . '" title="' . _GEN_ISSTICKY . '" /> - ' . _GEN_ISSTICKY . '' : '<img src="' . JB_URLEMOTIONSPATH . 'pushpin.gif" alt="' . _GEN_ISSTICKY . '" title="' . _GEN_ISSTICKY . '" /> - ' . _GEN_ISSTICKY . '';
								}
								?>
								<br/>
								<?php
								if($lockedForum == 1){
									echo $fbIcons->forumlocked ? '<img src="' . JB_URLICONSPATH . '' . $fbIcons->forumlocked . '" border="0" alt="' . _GEN_LOCKED_FORUM . '" /> - ' . _GEN_LOCKED_FORUM . '' : '  <img src="' . JB_URLEMOTIONSPATH . 'lock.gif" border="0"   alt="' . _GEN_LOCKED_FORUM . '" /> - ' . _GEN_LOCKED_FORUM . '';
									echo '<br />';
								} else{
									echo "";
								}
