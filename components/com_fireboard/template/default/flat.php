...
<tr class="fb_sth fbs ">
	...
	foreach($messages[0] as $leaf){
		...
		?>
	<tr class="<?php echo $boardclass ?><?php echo $tabclass[$k];?><?php if($leaf->ordering == 0){
	} else{
		echo '_stickymsg';
		$topicSticky = 1;
	}?>">
		...
		if($leaf->locked == 0){
			...
			echo $fbIcons->topiclocked ? '<td class="td-1" align="center"><img src="' . JB_URLICONSPATH . '' . $fbIcons->topiclocked . '" border="0" alt="' . _GEN_LOCKED_TOPIC . '" />' : '<img src="' . JB_URLEMOTIONSPATH . 'lock.gif"  alt="' . _GEN_LOCKED_TOPIC . '" title="' . _GEN_LOCKED_TOPIC . '" /></td>';
			$topicLocked = 1;
		}
		if($leaf->moved == 0){
			?>
			...
			if($leaf->ordering == 0){
				...
				echo "<td class=\"td-3\">";
				echo $fbIcons->topicsticky ? '<img  class="stickyicon" src="' . JB_URLICONSPATH . '' . $fbIcons->topicsticky . '" border="0" alt="' . _GEN_ISSTICKY . '" />' : '<img class="stickyicon" src="' . JB_URLEMOTIONSPATH . 'pushpin.gif"  alt="' . _GEN_ISSTICKY . '" title="' . _GEN_ISSTICKY . '" />';
				$topicSticky = 1;
			}
			//(JJ) ATTACHMENTS ICON
			if($attachmentsicon > 0){
				echo $fbIcons->topicattach ? '<img  class="attachicon" src="' . JB_URLICONSPATH . '' . $fbIcons->topicattach . '" border="0" alt="' . _FB_ATTACH . '" />' : '<img class="attachicon" src="' . JB_URLEMOTIONSPATH . 'attachment.gif"  alt="' . _FB_ATTACH . '" title="' . _FB_ATTACH . '" />';
			}
			?>
			<div class="fb-topic-title-cover">
				...
				if($fbConfig->allowfavorites == 1){
					...
					if(intval($database->loadResult()) > 0){
						echo $fbIcons->favoritestar ? '<img class="favoritestar" src="' . JB_URLICONSPATH . '' . $fbIcons->favoritestar . '" border="0" alt="' . _FB_FAVORITE . '" />' : '<img class="favoritestar" src="' . JB_URLEMOTIONSPATH . 'favoritestar.gif"  alt="' . _FB_FAVORITE . '" title="' . _FB_FAVORITE . '" />';
					}
				}
				?>
				...
				if($totalMessages > $fbConfig->messages_per_page){
					...
					for($hopPage = $startPage; $hopPage <= $threadPages; $hopPage++){
						...
						echo '<a href="' . sefRelToAbs(JB_LIVEURLREL . '&amp;func=view&amp;id=' . $leaf->id . '&amp;catid=' . $catid . '&amp;limit=' . $fbConfig->messages_per_page . '&amp;limitstart=' . (($hopPage - 1) * $fbConfig->messages_per_page)) . '">' . $hopPage . '</a>';
					}
					echo ("]</span>");
				}
				?>
		...
		<td class="td-6">
			<div class="fb-latest-subject-date fbs">
				...
				<a href="<?php echo sefRelToAbs('index.php?option=com_fireboard&amp;Itemid=' . $Itemid . '&amp;func=view&amp;catid=' . $catid . '&amp;id=' . $last_reply[$leaf->id]->id) . '#' . $last_reply[$leaf->id]->id;?>">
					<?php
					if(!$leaf->moved){
						echo $fbIcons->latestpost ? '<img src="' . JB_URLICONSPATH . '' . $fbIcons->latestpost . '" border="0" alt="' . _SHOW_LAST . '" />' : '  <img src="' . JB_URLEMOTIONSPATH . 'icon_newest_reply.gif" border="0"  alt="' . _SHOW_LAST . '" title="' . _SHOW_LAST . '" />';
					}
					?>
				</a>
			</div>
