<?php
/**
 * @version $Id: thread.php 462 2007-12-10 00:05:53Z fxstein $
 * Fireboard Component
 * @package Fireboard
...
function thread_flat(&$tree, &$leaves, $branchid = 0, $level = 0){
	foreach($leaves[$branchid] as $leaf){
		...
		$tree[] = $leaf;
		$GLOBALS['FB_c']++;
		if(is_array($leaves[$leaf->id])) thread_flat($tree, $leaves, $leaf->id, $level + 1);
	}
	return $tree;
}
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0" class="fb_blocktable">
							...
							<tr class="fb_threadview_row">
								...
								<td align="center" width="5"
									<?php echo $leaf->id == $id ? " class=\"" . $boardclass . "sectiontableentry2\">" : ">";
									if($leaf->ordering == 0){
										if($leaf->locked == 0){
											echo "&nbsp;";
										} else{
										...
										<tr>
											<td<?php echo $leaf->id == $id ? " class=\"" . $boardclass . "sectiontableentry2\"" : ""; ?>>
												...
												for($i = 0; $i < $leaf->level; $i++){
													if($array[$i] > 0) echo ($vert); elseif($array[$i] == 0) echo ($blank);
												}
												if($array[$leaf->level] > 0) echo ($join); elseif($array[$leaf->level] == 0 && $leaf->parent != 0) echo ($end);
												?>
											</td>
											<?php
								...
								<td align="center" <?php echo $leaf->id == $id ? ' class="' . $boardclass . 'sectiontableentry2"' : '';?>>
									<small><?php echo $leaf->email != "" && $my_id > 0 && $fbConfig->showemail == '1' ? '<a href="mailto:' . stripslashes($leaf->email) . '">' . stripslashes($leaf->name) . '</a>' : stripslashes($leaf->name); ?></small>
								</td>
								<td align="center" <?php echo $leaf->id == $id ? ' class=""' . $boardclass . 'sectiontableentry2"' : '';?>>
