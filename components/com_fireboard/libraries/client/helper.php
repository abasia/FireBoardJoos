...
class JClientHelper{
	...
	public static function getCredentials($client, $force = false){
		...
		if(!isset($credentials[$client]) || $force){
			...
			switch($client){
				case 'ftp':
					$options = array('enabled' => $config->getValue('config.ftp_enable'), 'host' => $config->getValue('config.ftp_host'), 'port' => $config->getValue('config.ftp_port'), 'user' => $config->getValue('config.ftp_user'), 'pass' => $config->getValue('config.ftp_pass'), 'root' => $config->getValue('config.ftp_root'));
					break;
	...
	function setCredentials($client, $user, $pass){
		...
		switch($client){
			case 'ftp':
				$config =& JFactory::getConfig();
				$options = array('enabled' => $config->getValue('config.ftp_enable'), 'host' => $config->getValue('config.ftp_host'), 'port' => $config->getValue('config.ftp_port'),);
	...
	function hasCredentials($client){
		...
		switch($client){
			case 'ftp':
				$config =& JFactory::getConfig();
				$options = array('enabled' => $config->getValue('config.ftp_enable'), 'user' => $config->getValue('config.ftp_user'), 'pass' => $config->getValue('config.ftp_pass'));
				break;
